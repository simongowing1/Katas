function calc(x) {
  ...
  for (let i = 0; i < total1.length; i += 1) {
    // add the numerical value of the index to the 'sumTotal1' variable
    sumTotal1 += parseFloat(total1[i]);
  }
  // create new variable using .replace() to replace instances of the
  // number 7 with the number 1 in the total1 string
  total2 = total1.replace(/7/g, 1);
  // iterate over the new string 'total2'
  for (let i = 0; i < total2.length; i += 1) {
    // add the numerical value of the index to the 'sumTotal2' variable
    sumTotal2 += parseFloat(total2[i]);
  }
